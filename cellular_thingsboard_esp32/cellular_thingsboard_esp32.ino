...
void setup() {
  ...
  pinMode(OE_ENABLE,OUTPUT);
  digitalWrite(OE_ENABLE,HIGH);
  ...
  while (true) {
  if((millis() - modemResetTimer >= rstTime)||(modemFailCount > 10)){
      systemState = 0xFF;
      modemFailCount = 0;
    ...
    switch (systemState) {
      case 0:               //Initialize Modem
        ...
        if (!sim7000.setBaudRate(3686400)) {
        ...
        //-----------------------------------------------------------//
        if (!sim7000.getIMEI(receiveBuffer)) {
        ...
        //-----------------------------------------------------------//
        if (!sim7000.setPreferredLTEMode(3)) {
